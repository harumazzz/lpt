...
class VietnamCalendar {
  static int jdFromDate(int dd, int mm, int yy) {
    final a = ((14 - mm) / 12).toInt();
    final y = yy + 4800 - a;
    final m = mm + 12 * a - 3;
    var jd = (dd + (153 * m + 2) / 5 + 365 * y + y / 4 - y / 100 + y / 400 - 32045).toInt();
    if (jd < 2299161) {
      jd = (dd + (153 * m + 2) / 5 + 365 * y + y / 4 - 32083).toInt();
    }
    return jd;
  }
  ...
  static (int, int, int) jdToDate(int jd) {
    final (b, c) = (() {
      if (jd > 2299160) {
        var a = jd + 32044;
        var b = ((4 * a + 3) / 146097).toInt();
        var c = (a - (b * 146097) / 4).toInt();
        return (b, c);
      } else {
        return (0, jd + 32082);
      }
    }());
    final d = ((4 * c + 3) / 1461).toInt();
    final e = (c - (1461 * d) / 4).toInt();
    final m = ((5 * e + 2) / 153).toInt();
    final day = (e - (153 * m + 2) / 5 + 1).toInt();
    final month = (m + 3 - 12 * (m / 10)).toInt();
    final year = (b * 100 + d - 4800 + m / 10).toInt();
  ...
  static double newMoonAA98(int k) {
    ...
    final deltat =
        (() {
          if (T < -11) {
            return 0.001 + 0.000839 * T + 0.0002261 * t2 - 0.00000845 * t3 - 0.000000081 * T * t3;
          } else {
  ...
  static int getLeapMonthOffset(int a11, double timeZone) {
    ...
    var i = 1;
    var arc = integerOf(getSunLongitude(getNewMoonDay(k + i, timeZone), timeZone) / 30);
    do {
      last = arc;
      i++;
      arc = integerOf(getSunLongitude(getNewMoonDay(k + i, timeZone), timeZone) / 30);
    } while (arc != last && i < 14);
    return i - 1;
  }

  static (int, int, int, int) convertSolar2Lunar(int dd, int mm, int yy, double timeZone) {
  ...
  static (int, int, int) convertLunar2Solar(
    ...
    final (a11, b11) = (() {
      if (lunarMonth < 11) {
        return (getLunarMonth11(lunarYear - 1, timeZone), getLunarMonth11(lunarYear, timeZone));
      } else {
        return (getLunarMonth11(lunarYear, timeZone), getLunarMonth11(lunarYear + 1, timeZone));
      }
    }());
    final k = integerOf(0.5 + (a11 - 2415021.076998695) / 29.530588853);
